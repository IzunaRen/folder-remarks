...
function buildRemarkedTreeRoots(args) {
    const byId = new Map();
    const roots = [];
    const getOrCreate = (id, name, key) => {
        const existing = byId.get(id);
        if (existing)
            return existing;
        const node = { id, name, key, children: [] };
        byId.set(id, node);
        return node;
    };
    const ensureChild = (parent, seg, key) => {
